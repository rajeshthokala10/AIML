...
class EntityExtractor:
    ...
    def extract(self, query: str) -> list[ExtractedEntity]:
        ...
        for pattern, etype in EQUIPMENT_PATTERNS:
            for m in re.finditer(pattern, query, re.IGNORECASE):
                entities.append(ExtractedEntity(
                    entity_type=etype, value=m.group(1),
                    span=(m.start(), m.end()),
                    normalized=m.group(1).upper().replace(" ", "-"),
                ))

        for pattern, etype in PART_NUMBER_PATTERNS:
            for m in re.finditer(pattern, query, re.IGNORECASE):
                entities.append(ExtractedEntity(
                    entity_type=etype, value=m.group(1),
                    span=(m.start(), m.end()),
        ...
        for key, normalized in SUPPLIER_NAMES.items():
            idx = query_lower.find(key)
            if idx >= 0:
                entities.append(ExtractedEntity(
                    entity_type="supplier", value=query[idx:idx+len(key)],
                    span=(idx, idx + len(key)),
                    normalized=normalized,
                ))

        for key, normalized in METRIC_NAMES.items():
            pattern = r"\b" + re.escape(key) + r"\b"
            for m in re.finditer(pattern, query_lower):
                entities.append(ExtractedEntity(
                    entity_type="metric", value=m.group(0),
                    span=(m.start(), m.end()),
                    normalized=normalized,
                ))

        for key, normalized in DEPARTMENT_NAMES.items():
            pattern = r"\b" + re.escape(key) + r"\b"
            m = re.search(pattern, query_lower)
            if m:
                entities.append(ExtractedEntity(
                    entity_type="department", value=m.group(0),
                    span=(m.start(), m.end()),
                    normalized=normalized,
                ))

        for pattern, etype in PLANT_PATTERNS:
            for m in re.finditer(pattern, query, re.IGNORECASE):
                entities.append(ExtractedEntity(
                    entity_type=etype, value=m.group(1),
                    span=(m.start(), m.end()),
        ...
        for pattern, etype in STANDARD_PATTERNS:
            for m in re.finditer(pattern, query, re.IGNORECASE):
                entities.append(ExtractedEntity(
                    entity_type=etype, value=m.group(1),
                    span=(m.start(), m.end()),
        ...
        for pattern, etype in DATE_PATTERNS:
            for m in re.finditer(pattern, query, re.IGNORECASE):
                entities.append(ExtractedEntity(
                    entity_type=etype, value=m.group(0).strip(),
                    span=(m.start(), m.end()),
                    normalized=m.group(0).strip(),
                ))

        for pattern, etype in MATERIAL_PATTERNS:
            for m in re.finditer(pattern, query, re.IGNORECASE):
                entities.append(ExtractedEntity(
                    entity_type=etype, value=m.group(0),
                    span=(m.start(), m.end()),
                    normalized=m.group(0).title(),
                ))

        for pattern, etype in SEVERITY_PATTERNS:
            for m in re.finditer(pattern, query, re.IGNORECASE):
                entities.append(ExtractedEntity(
                    entity_type=etype, value=m.group(0),
                    span=(m.start(), m.end()),
